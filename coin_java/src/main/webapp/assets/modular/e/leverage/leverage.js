...
Leverage.initColumn = function () {
    return [[
        // {type: 'checkbox'},
         {field: 'leverageId', hide: true, sort: true, title: 'id'},
    ...
    Leverage.search = function () {
        ...
        queryData['leverageId'] = Leverage.condition.leverageId;
...
Leverage.openLeverageDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/leverage/leverage_edit/' + Leverage.seItem.id
        });
    ...
    Leverage.onDeleteLeverage = function (data) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/leverage/delete", function () {
                Feng.success("删除成功!");
                table.reload(Leverage.tableId);
            }, function (data) {
                Feng.error("删除失败!" + data.responseJSON.message + "!");
            });
            ajax.set("leverageId", data.leverageId);
            ajax.start();
        };
    ...
    Leverage.openAddLeverage = function () {
        ...
        top.layui.admin.open({
            ...
            title: '添加杠杆倍率',
            area:["800px","420px"],
            content: Feng.ctxPath + '/leverage/leverage_add',
            end: function () {
     ...
     */
     Leverage.onEditLeverage = function (data) {
        ...
        top.layui.admin.open({
            ...
            title: '修改杠杆倍率',
            area:["800px","420px"],
            content: Feng.ctxPath + '/leverage/leverage_edit?leverageId=' + data.leverageId,
            end: function () {
                admin.getTempData('formOk') && table.reload(Leverage.tableId);
            }
        });
 ...
 // 渲染表格
    var tableResult = table.render({
        ...
        url: Feng.ctxPath + '/leverage/list',
        page: true,
        height: "full-158",
        cellMinWidth: 100,
        limit:100,
