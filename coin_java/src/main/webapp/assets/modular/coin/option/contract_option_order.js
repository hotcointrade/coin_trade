layui.use(['table', 'admin', 'ax', 'ztree','laydate','form'], function () {
    ...
    Obj.initColumn = function () {
        return [[
            ...
            {field: 'secondsYield',  title: '结算周期/赢收益率/输赔率',minWidth: 80,templet:function (data) {
                    return data.secondsYield+"";
                }},
            ...
            {field: 'recordNum',  title: '上返佣金额',minWidth: 80,templet:function (data) {
                if(data.orRecord == "Y"){
            ...
            {field: 'recordNum',  title: '上上返佣金额',minWidth: 120,templet:function (data) {
                    if(data.orTowRecord == "Y"){
                        return data.recordTowNum
                    }else {
                        return '未返佣'
                    }
                }},
            ...
            {field: 'createTime', sort: true, title: '交易时间',minWidth: 150},
     ...
     *
     */
    Obj.optionCtrl = function (data,flag) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/"+apiPrefix+"/jiesuan", function (data) {
                if(data.success){
                    Feng.success("操作成功!");
                }else{
            ...
            ajax.set("id", data.id);
            ajax.set("ctrlNum", flag);
            ajax.start();
        };
        if(flag){
            Feng.confirm("是否人工控制赢",operation);
        }else{
            Feng.confirm("是否人工控制输", operation);
        }
    };
    /**
     * 一键佩服
     *
     */
    Obj.onBack = function (data) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/"+apiPrefix+"/back", function (data) {
                if(data.success){
                    Feng.success("返还成功!");
                }else{
                    Feng.error("返还失败!" + data.message + "!");
                }
                table.reload(Obj.tableId);
            }, function (data) {
                Feng.error("返还失败!" + data.message + "!");
        ...
        Feng.confirm("是否赔付用户"+data.memberIdValue+",获奖金额："+data.rewardAmount+",手续费："+data.fee+"?", operation);
    };
     ...
     // 搜索按钮点击事件
        ...
        table.on('tool(' + Obj.tableId + ')', function (obj) {
            ...
            if (layEvent === 'edit') {
                ...
                Obj.onBack(data);
            }else if (layEvent === 'optionCtrlY') {
                Obj.optionCtrl(data,'Y');
            }else if (layEvent === 'optionCtrlN') {
                Obj.optionCtrl(data,'N');
            }
        });
});
