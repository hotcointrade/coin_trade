layui.use(['table', 'admin', 'ax', 'ztree','laydate','form'], function () {
    ...
    Obj.initColumn = function () {
        return [[
            ...
            {field: 'oods',  title: '赢赔率',templet:function (data) {
                    return data.oods+"%";
                }},
            {field: 'oous',  title: '输赔率',templet:function (data) {
                    return data.oous+"%";
                }},
            {field: 'secondsYield',  title: '结算周期/赢收益率/输赔率',minWidth: 150,templet:function (data) {
                    return data.secondsYield+"";
                }},
            {field: 'rods',  title: '上级返佣',templet:function (data) {
                    return data.rods+"%";
                }},
            {field: 'rods',  title: '上上级返佣',templet:function (data) {
                    return data.rodsTow+"%";
                }},
            ...
            {field: 'status',fixed: 'right', templet: '#ctrlUpTpl', title: '控制涨'},
            {field: 'status',fixed: 'right', templet: '#ctrlDownTpl', title: '控制跌'},
            {fixed: 'right',align: 'center', toolbar: '#tableBar', title: '操作', minWidth: 140}
        ]];
    };
    ...
    Obj.changeStatus = function (id, checked) {
        if (checked) {
            var ajax = new $ax(Feng.ctxPath +"/"+apiPrefix+'/ctrlUp/Y', function (data) {
                if(data.success){
                    Feng.success("已启用!");
                }else{
            ...
            ajax.set("optionId", id);
            ajax.start();
        } else {
            var ajax = new $ax(Feng.ctxPath + "/"+apiPrefix+"/ctrlUp/N", function (data) {
                if(data.success){
                    Feng.success("已启用!");
                } else{
                    Feng.error("失败!"+data.message);
     ...
     * @param checked 是否选中（true,false），选中就是启用，未选中就是禁用
     */
    Obj.changeStatus2 = function (id, checked) {
        if (checked) {
            var ajax = new $ax(Feng.ctxPath +"/"+apiPrefix+'/ctrlDown/Y', function (data) {
                if(data.success){
                    Feng.success("已启用!");
                }
                else{
            ...
            var ajax = new $ax(Feng.ctxPath + "/"+apiPrefix+"/ctrlDown/N", function (data) {
                if(data.success){
                    Feng.success("已启用!");
                } else{
                    Feng.error("失败!"+data.message);
