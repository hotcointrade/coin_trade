...
Currency.openCurrencyDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/finOption/currency_edit/' + Currency.seItem.id
        });
    ...
    Currency.onDeleteCurrency = function (data) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/finOption/delete", function () {
                Feng.success("删除成功!");
        ...
        Feng.confirm("是否删除期权账户 ?", operation);
    };
    ...
    Currency.openAddCurrency = function () {
        ...
        top.layui.admin.open({
            ...
            title: '添加期权账户',
            area:["800px","420px"],
            content: Feng.ctxPath + '/finOption/currency_add',
            end: function () {
     ...
     */
     Currency.onEditCurrency = function (data) {
        ...
        top.layui.admin.open({
            ...
            title: '修改期权账户',
            area:["800px","420px"],
            content: Feng.ctxPath + '/finOption/currency_edit?currencyId=' + data.currencyId,
            end: function () {
                admin.getTempData('formOk') && table.reload(Currency.tableId);
            }
        });
 ...
 // 渲染表格
    var tableResult = table.render({
        ...
        url: Feng.ctxPath + '/finOption/list',
        page: true,
        height: "full-158",
        cellMinWidth: 100,
        limit:100,
