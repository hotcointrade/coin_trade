...
public class SystemMatchService extends Constant {
    ...
    Match createMatch(Match match) {
        ...
        if (StrUtil.equals(match.getDealWay(), DealWay.LIMIT.code)) {//市价，拉取最新价
            ...
            Depth o = redisUtil.getBean(DEPTH + symbols, Depth.class);

            List<JSONArray> asks = (List<JSONArray>) o.getAsks();
            List<JSONArray> bids = (List<JSONArray>) o.getBids();
            ...
            if (match.getMatchType().equals(MatchType.SELL.code)) {//卖
                ...
                for (JSONArray ask : asks) {
                    BigDecimal b = new BigDecimal(ask.get(0).toString());
            ...
            if (match.getMatchType().equals(MatchType.BUY.code)) {//买
                for (JSONArray bid : bids) {
                    log.info("bid[0]: " + bid.get(0));
                    BigDecimal b = new BigDecimal(bid.get(0).toString());
