...
public class F implements IPromotionFactory {
	...
	private final MemberMapper appMemberMapper = SpringContextHolder.getBean(MemberMapper.class);
	
	private final ConfigMapper configMapper = SpringContextHolder.getBean(ConfigMapper.class);
	
	private final GatewayRecordMapper gatewayRecordMapper = SpringContextHolder.getBean(GatewayRecordMapper.class);
	
	private final GatewayDefineMapper gatewayDefineMapper = SpringContextHolder.getBean(GatewayDefineMapper.class);
	
	private final WalletMapper walletMapper = SpringContextHolder.getBean(WalletMapper.class);
	private final RegionMapper regionMapper = SpringContextHolder.getBean(RegionMapper.class);
	
	private final WalletHistoryMapper walletHistoryMapper = SpringContextHolder.getBean(WalletHistoryMapper.class);
	
	private final CashflowMapper cashflowMapper = SpringContextHolder.getBean(CashflowMapper.class);
	
	private final SvcExceptionMapper svcExceptionMapper = SpringContextHolder.getBean(SvcExceptionMapper.class);
	
	private final ContractMapper contractMapper = SpringContextHolder.getBean(ContractMapper.class);
	private final CurrencyMapper currencyMapper = SpringContextHolder.getBean(CurrencyMapper.class);
	private final FinMiningMapper finMiningMapper = SpringContextHolder.getBean(FinMiningMapper.class);
	private final FinOptionMapper finOptionMapper = SpringContextHolder.getBean(FinOptionMapper.class);
	private final FinFuturesMapper finFuturesMapper = SpringContextHolder.getBean(FinFuturesMapper.class);
	private final LegalMapper legalMapper = SpringContextHolder.getBean(LegalMapper.class);
	
	private final DappMapper dappMapper = SpringContextHolder.getBean(DappMapper.class);
	
	public RedisUtil redisUtil = SpringContextHolder.getBean(RedisUtil.class);
	
	private final SpotMapper spotMapper = SpringContextHolder.getBean(SpotMapper.class);
	private final SwapMapper swapMapper = SpringContextHolder.getBean(SwapMapper.class);
	private final FuturesMapper futuresMapper = SpringContextHolder.getBean(FuturesMapper.class);
	//private OptionMapper optionMapper = SpringContextHolder.getBean(OptionMapper.class);
	private final ContractOptionCoinMapper optionCoinMapper = SpringContextHolder.getBean(ContractOptionCoinMapper.class);
	private final UserMapper userMapper = SpringContextHolder.getBean(UserMapper.class);
	private final MiningOrderMapper miningOrderMapper = SpringContextHolder.getBean(MiningOrderMapper.class);
	private final MiningMapper miningMapper = SpringContextHolder.getBean(MiningMapper.class);
	
	private final WithdrawService withdrawService = SpringContextHolder.getBean(WithdrawService.class);
	private final RechargeService rechargeService = SpringContextHolder.getBean(RechargeService.class);
...
//        Object o = redisUtil.get(Constant.configPrx + code);
//        if(o!=null){
//            return o.toString();
//        }
		...
		if (result != null) {
			// redisUtil.set(Constant.configPrx + code,result.getValue());
