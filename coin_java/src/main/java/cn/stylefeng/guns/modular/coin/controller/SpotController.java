   ...
   // @BussinessLog(value = "删除现货交易对", key = "spotId", dict = SpotMap.class)
    ...
    private void updateWalletAndCurrency(String type,String coin) {
        ...
        if ("delete".equals(type)) {
            ...
            FinMining finMining1 = new FinMining();
            finMining1.setDel("N");
            finMining1.setType(coin);
            List<FinMining> finMiningsList = finMiningService.list(new QueryWrapper<>(finMining1));
            for (FinMining finMining : finMiningsList) {
                finMining.setDel("Y");
                finMining.setUpdateTime(new Date());
                finMining.setUpdateUser(1l);
            ...
            for (Member m :memberList){
                ...
                walletone.setType(coin);
                walletone.setDel("Y");
                List<Wallet> walletList = walletService.list(new QueryWrapper<>(walletone));
                for (Wallet wallet1 : walletList) {
                    wallet1.setDel("N");
                    wallet1.setUsedPrice(BigDecimal.ZERO).setLockedPrice(BigDecimal.ZERO);
                    wallet1.setUpdateTime(new Date());
                    wallet1.setUpdateUser(1l);
                    walletService.updateById(wallet1);
                }
                ...
                currencyone.setDel("Y");
                currencyone.setType(coin);
                List<Currency> currencyList = currencyService.list(new QueryWrapper<>(currencyone));
                for (Currency currency1 : currencyList) {
                    currency1.setDel("N");
                    currency1.setUsedPrice(BigDecimal.ZERO);
                    currency1.setLockedPrice(BigDecimal.ZERO);
                    currency1.setUpdateTime(new Date());
                    currency1.setUpdateUser(1l);
                    currencyService.updateById(currency1);
                }
                ...
                FinMining finMiningone = new FinMining();
                finMiningone.setDel("Y");
                finMiningone.setType(coin);
                List<FinMining> finMiningsList = finMiningService.list(new QueryWrapper<>(finMiningone));
                for (FinMining finMining : finMiningsList) {
                    finMining.setDel("N");
                    finMining.setUpdateTime(new Date());
                    finMining.setUsedPrice(BigDecimal.ZERO);
                    finMining.setLockedPrice(BigDecimal.ZERO);
                    finMining.setUpdateUser(1l);
                    finMiningService.updateById(finMining);
                }

                if (finMiningsList.size()==0) {
