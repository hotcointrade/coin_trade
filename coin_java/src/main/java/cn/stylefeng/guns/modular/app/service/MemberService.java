...
//        BigDecimal otcBuy  =BigDecimal.ZERO;
//        List<Buy> list = buyService.list(new QueryWrapper<>(buy));
//        for (Buy buy1 : list) {
//            otcBuy =otcBuy.add(buy1.getNumber());
//        }
//        BigDecimal otcBuy1 = (BigDecimal)map.get("otcBuy");
//        map.put("otcBuy",otcBuy.add(otcBuy1).setScale(4, BigDecimal.ROUND_HALF_UP));
//
//        Sell sell1 = new Sell();
//        sell1.setCoin(type);
//        sell1.setMemberId(memberId);
//        BigDecimal otcSell  =BigDecimal.ZERO;
//        List<Sell> list1 = sellService.list(new QueryWrapper<>(sell1));
//        for (Sell sell : list1) {
//            otcSell = otcSell.add(sell.getNumber());
//        }
//        BigDecimal otcSell1 = (BigDecimal)map.get("otcSell");
//        map.put("otcSell",otcSell.add(otcSell1).setScale(4, BigDecimal.ROUND_HALF_UP));
...
//        map.put("otcCount",count-backCount);
     ...
     * 查询用户信息
     */
    public Page<Map<String,Object>> selectByCondition(String condition,Long refereeId,Long memberId,String recommendIds,String uid) {
        Page page = LayuiPageFactory.defaultPage();
        return this.baseMapper.selectByConditionUid(page,condition,refereeId,memberId,recommendIds,uid);
    }
    /**
     * 查询用户信息
     */
    ...
    public Page<Map<String,Object>> selectAllForTeam(String condition,Long refereeId,Long userId,Page page) {
        ...
        for (Map<String, Object> record : records) {
            ...
            if (account.contains("@")){
                ...
                record.put("account",account.substring(0,3)+"****"+account.substring(account.length()-2,account.length()-0));
