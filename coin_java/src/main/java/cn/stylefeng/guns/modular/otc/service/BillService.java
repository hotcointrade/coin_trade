...
public class BillService extends ServiceImpl<BillMapper, Bill>
{
    ...
    public ResponseData fail2(Long billId)
    {
        Bill bill = this.baseMapper.selectById(billId);

        if (ProConst.BillStatus.FINISH.code.equals(bill.getStatus()))
            return ResponseData.error(500, "订单已发生更新，请重新操作");
        if (ProConst.BillStatus.CANCEL.code.equals(bill.getStatus()))
            return ResponseData.error(500, "订单已发生更新，请重新操作");
...
//                .setAppealStatus(ProConst.AppealStatus.CANCEL.code)
    ...
    public ResponseData pass(Long billId)
    {
        ...
        if(!"Y".equals(buy.getValidStatus())){
            buy.setValidStatus("Y");
            memberService.updateById(buy);
            //有效用户之后 进行奖励
            homeService.calculateReferralCommission(buy);
            Object o = redisUtil.get(Constant.SINGLE_ACCOUNT + buy.getAccount());
            if(o!=null) {
                String token = (String) o;
                redisUtil.set(token, buy, Long.parseLong(F.me().cfg(Constant.TOKEN_EXPIRE)));
            }
        }
    ...
    @Transactional(rollbackFor = Exception.class)
    public ResponseData pass2(Long billId)
    {
        Bill entity = this.baseMapper.selectById(billId);

        if (ProConst.BillStatus.FINISH.code.equals(entity.getStatus()))
            return ResponseData.error(500, "订单已发生更新，请重新操作");
        if (ProConst.BillStatus.CANCEL.code.equals(entity.getStatus()))
            return ResponseData.error(500, "订单已发生更新，请重新操作");
//        if (StrUtil.isBlank(entity.getDuty()))
//            return ResponseData.error(500, "请先判责，再放币");
