<template>
	<div class="home_div">
				...
				<section class="home-top">
					...
					<div class="div_inquiry-logo">
						...
						<!-- <div class="inquiry-input">
							<input type="text" v-model="username" :placeholder="$t('home.huobi92')">
							<button @click="checkUser" style="margin-left: 10px;">
								<img src="../assets/sous.png"
									style="margin: 0px 4px;width: 28px;height: 28px;margin-left: -10px;" />
								<i style="font-size: 18px;position: relative;top: 4px;"> {{ $t("home.huobi93") }}</i>
							</button>
						</div> -->
					</div>
					...
					<el-dialog :visible.sync="visible" width="30%">
						<div data-v-c6f7164c="" class="inquiry-channel">
							...
							<dl data-v-c6f7164c="" class="dl-first">
								...
								<dd data-v-c6f7164c="" class="text-desc">{{ $t("home.huobi94") }} <em> {{ username }}
									</em> {{ msg }}</dd>
							</dl>
							<dl data-v-c6f7164c="" class="dl-second">
								<dt data-v-c6f7164c="">{{ $t("home.huobi95") }}</dt>
								<dd data-v-c6f7164c="">{{ $t("home.huobi96") }}</dd>
							</dl>
							<div data-v-c6f7164c="" class="btn-action">
								<button data-v-c6f7164c="" @click="visible = false">{{ $t("home.huobi97") }}</button>
							</div>
						</div>
					</el-dialog>

					<div class="block"
						style="position: relative;right: 0px;left: 0px;text-align: center;border-radius: 30px;top: 190px;width: 1000px;height: 280px;">
						<el-carousel :interval="3000" type="card" indicator-position="none" arrow="never"
							height="height: 280px;">
							<el-carousel-item v-for="item in bannerArr" :key="item.id">
								<a :href="item.link == '' ? 'javascript:;' : item.link"
		...
		<div class="d_notice_box">
			<div @click="detailFun2(item)" class="d_notice_item" v-for="(item, index) in noticeArr"
				:key="item.carouselId">
		...
		<div data-v-63c57342="" class="feature_wrap">
			<h2 data-v-63c57342="" style="font-weight: 400">
				...
				<b> $263,275,{{ testNum }}</b> {{ $t("home.huobi2") }}
			</h2>
			<!-- <p data-v-63c57342="">
		...
		<div class="container" style="width: 70%;">
			...
			<div class="ainer_maer">
				<div class="el-table table el-table--fit el-table--enable-row-hover el-table--enable-row-transition">
					<el-table border stripe :data="marketList" @row-click="detailFun" style="width: 100%;"
						...
						<el-table-column align="center" prop="open" :label="$t('home.huobi11')" width="220px">
							<template slot-scope="scope">
								<span style="font-weight: 400; font-size: 14px">{{ scope.row.close }}
									<!-- <span style="font-weight: 400; color:#afafaf; font-size: 14px">
										≈ {{ (scope.row.close * rateNumber).toFixed(scope.row.number) }}
										{{ rateObj.rateName }}</span> -->
										<span style="font-weight: 400; color:#afafaf; font-size: 14px">
				...
				<div class="ticker-more" style="
						...
						">
					<a href="/#/quotation">
						<span class="text" data-lang-key="view.newHome.quotation.more">{{ $t("home.huobi23") }}
							></span>
					</a>
				</div>
			</div>
			...
			<div class="new_appDownload_container">
				...
				<div class="new_appDownload_link">
					<div class="new_appDownload_link_title">
						<span class="one" style="font-size: 20px;line-height: 30px; font-weight: 700">{{
							$t("home.huobi16")
						}}</span>
					</div>
					<div class="new_appDownload_link_title">
		...
		<div v-if="isToken == undefined" class="new_registered">
			<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="100%"
				height="300">
				<g fill="#fdfdfdeb" transform="translate(-976.065 0)">
					<path
						d="M 0 100 Q 250 35, 500 100 T 1000 100 T 1500 100 T 2000 100 T 2500 100 T 3000 100 T 3500 100 T 4000 100 T 4500 100 T 5000 100 T 5500 100 T 6000 100 V 300 H 0 V 0">
					</path>
					<animateTransform attributeName="transform" attributeType="XML" type="translate" from="0" to="-2000"
						dur="10s" repeatCount="indefinite"></animateTransform>
				</g>
				<g fill="#fff" transform="translate(-1561.7 0)">
					<path
						d="M 0 100 Q 400 35, 800 100 T 1600 100 T 2400 100 T 3200 100 T 4000 100 T 4800 100 T 5600 100 T 6400 100 T 7200 100 V 300 H 0 V 0">
					</path>
					<animateTransform attributeName="transform" attributeType="XML" type="translate" from="0" to="-3200"
						dur="10s" repeatCount="indefinite"></animateTransform>
				</g>
			</svg>
			<h4 v-if="isToken == undefined" class="reg_title color-gold" style="font-weight: 600">
				{{ $t("home.huobi35") }}
			</h4>
			<p v-if="isToken == undefined" class="ppm_cc_cursor">{{ $t("home.huobi20") }}</p>
			<div v-if="isToken == undefined" class="new_registered_input">
...
.new_appDownload .new_appDownload_container .new_appDownload_link .new_appDownload_link_title.btn {
	font-size: 13px;
	font-weight: normal;
	margin-top: 50px;
}

.new_appDownload .new_appDownload_container .new_appDownload_link .new_appDownload_link_title {
	color: #dbdbdc;
	font-size: 28px;
	font-weight: 500;
	display: -webkit-flex;
...
.new_appDownload .new_appDownload_container .new_appDownload_link .new_appDownload_link_title.btn a {
	height: 35px;
	padding: 0 20px;
	-webkit-border-radius: 3px;
	-moz-border-radius: 3px;
...
.new_appDownload .new_appDownload_container .new_appDownload_link .new_appDownload_link_qrCode {
	display: flex;
	display: -webkit-flex;
	justify-content: space-between;
	margin-top: 50px;
...
.new_appDownload .new_appDownload_container .new_appDownload_link .new_appDownload_link_qrCode .new_appDownload_link_qrCode_left {
	flex: 1;
	font-size: 12px;
	text-align: end;
}

.new_appDownload .new_appDownload_container .new_appDownload_link .new_appDownload_link_qrCode .new_appDownload_link_qrCode_left p:nth-child(2) {
	margin-top: 20px;
	cursor: pointer;
}

.new_appDownload .new_appDownload_container .new_appDownload_link .new_appDownload_link_qrCode #qrcode {
	width: 100px;
	margin-left: 38px;
}
