	...
	<div class="wechat_div">
		<el-row :gutter="60">
			<el-breadcrumb separator-class="el-icon-arrow-right">
				...
				<el-breadcrumb-item>{{ $t('payMethods.wechat') }}</el-breadcrumb-item>
			</el-breadcrumb>
			...
			<el-col :xs="24" :sm="24" :md="12" :lg="12">
				<el-form ref="form" :model="form">
					...
					<el-form-item :label="$t('nav.account')" prop="account"
						:rules="[{ required: true, message: $t('payMethods.wechatInput') }]">
						<el-input v-model="form.account" autocomplete="off" :placeholder="$t('payMethods.wechatInput')">
						</el-input>
					</el-form-item>

					<h3>{{ $t('payMethods.wechatQrcode') }}({{ $t('payMethods.select') }})</h3>
