	...
	<div class="mstarch2">
		<div class="mstarch">
			...
			<div class="section-head">
				<div class="cl" style="position: relative; z-index: 1;">
					<div role="banner" aria-label="背景大图">
						<div class="home-banner-wrapper">
							<div class="home-banner">
								<video poster="@/starchainminer/static/image/earth_v6.jpg" autoplay="true" loop="true"
									muted="muted">
									<source type="video/mp4"
										src="https://lf9-static.bytednsdoc.com/obj/eden-cn/uhbfnupkbps/video/earth_v6.mp4">
									<!-- <source src="@/starchainminer/static/videos/MyVideo_1.mp4" type="video/mp4"></source> -->
								</video>
							</div>
						</div>
					</div>
					...
					<el-tabs v-model="activeName" @tab-click="handleClick">
						<el-tab-pane :label="$t('bico.W211')" name="first">
							...
							<div class="contentmep3">
								<el-row :gutter="20">
									<el-col :span="10" v-show="miningPage.records.length > 0"
										v-for="(item, index) in miningPage.records" :key="item.id">
										<div >
											<!-- 矿机商城模板示例开始 -->
											<div class="card-primer cloud-card-shadow gray-bg cloud-shadow-dark">
												<p class="card-btc com-font">
													<span class="icon-32 icon-32-btc">
														<img class="theme-night-display-hide pc-header-logo icon-32"
															:src="item.image2">
												...
												<p class="card-p com-font">
													...
													<span class="fs18">{{ item.fuelEnergyNumber }}
														{{ item.fuelEnergyCoin }}</span>
												</p>

												<p class="card-p com-font">
													<span class="card-span">{{ $t('bico.W216') }}：</span>
													<span class="fs18">{{ item.calculatingPowerNumber }}
														{{ item.calculatingPowerCoin }}</span>
												</p>
												...
												<button v-if="item.tradedAmount >= item.totalSupply"
													class="card_btns com-border com-color0 cp">{{ $t('bico.W218') }}</button>
												<button v-if="item.tradedAmount < item.totalSupply"
													@click="showDetail(item)"
													class="card_btns com-border com-color0 cpd">{{ $t('bico.W219') }}</button>
												<li class="card-line">
													<!--<p class="line-lines"></p>-->
													<!--<p class="line-red" style="width: calc(0px);"></p>-->
													<div style="width: 130px">
														<el-progress :stroke-width="8"
															:percentage="Number(item.tradedAmount / item.totalSupply * 100).toFixed(0)"></el-progress>
													</div>

													<span>{{ $t('bico.W220') }}：
														{{ item.tradedAmount }}/{{ item.totalSupply }}</span>
												</li>
											</div>
											<!-- 矿机商城模板示例开始 -->
										</div>
							...
							<el-pagination @current-change="getMiningList" :currentPage="miningPage.current"
								:page-size="miningPage.size" layout="prev, pager, next" :prev-text="$t('bico.W115')"
								:next-text="$t('bico.W116')" :total="miningPage.total">
							</el-pagination>
							<div style="height: 200px;"></div>
							<!-- 矿机商城列表结束 -->
						</el-tab-pane>
						...
						<el-tab-pane :label="$t('bico.W221')" name="second">
							<div class="contentmep4" v-show="miningOrderPage.records.length > 0"
								v-for="(item, index) in miningOrderPage.records" :key="item.id">
									...
									<div class="cloud-detail cloud-shadow-dark block-bg">
										...
										<div class="cloud-detail-row">
											<div class="cloud-row-left">
												<div class="row-left-top">
													...
													<div>
														<div class="fs33">{{ item.cycleNumber }}
															<span class="fs16 gray-text">
																{{ cycleTypeMap[item.cycleType] }}</span>
														</div>
													...
													<div>
														<div class="fs33">{{ item.fuelEnergyNumber }}
															<span
																class="fs16 gray-text">{{ item.fuelEnergyCoin }}/l</span>
														</div>
													...
													<div>
														<div class="fs33">{{ item.energyLossNumber }}
															<span
																class="fs16 gray-text">{{ item.energyLossCoin }}/s</span>
														</div>
												...
												<div class="row-left-bottom gray-bg">
													<div>
														...
														<span style="width: 100px;">{{ item.miningedDays }}
															{{ $t('bico.W214') }}</span>
													</div>
													<div>
											...
											<div class="cloud-row-right">
												...
												<div style="margin-top: -80px;">{{ $t('bico.W231') }}：<span
														class="red-text">
														<span class="fs33">{{ item.miningCoin }}</span>
													</span>
												</div>

												<div style="margin-top: -20px;">{{ $t('bico.W232') }}：<span
														class="red-text">
														<span class="fs333">{{ item.price }}</span> {{ item.priceCoin }}
													</span>
												</div>

												<div class="go-buy-selled POSr"
													@click="$router.push('/currencyTrade?symbol=' + item.energyLossCoin + '%2FUSDT')">
													<span class="icon-32 icon-32-btc" style="width: 24px;height: 24px;">
														<img class="theme-night-display-hide pc-header-logo icon-322"
															src="@/starchainminer/static/image/chongzhi.png">
												...
												<div class="go-buy-selled POSrr" @click="startUp(item)">
													...
													<p class="card-btc2 com-font" v-if="item.miningStatus == '0'">
														<span class="icon-32 icon-32-btc"
															...
															<img class="theme-night-display-hide pc-header-logo icon-322"
																src="@/starchainminer/static/image/zanting.png">
														</span>&ensp;
													</p>
													...
													<p class="card-btc2 com-font" v-if="item.miningStatus == '2'">
														<span class="icon-32 icon-32-btc"
															style="width: 24px;height: 24px;">
															<img class="theme-night-display-hide pc-header-logo icon-322"
																src="@/starchainminer/static/image/kaishi.png">
														</span>&ensp;&ensp;
													</p>
													<!-- 进行中 -->
													<p class="card-btc2 com-font flash animated infinite"
														v-if="item.miningStatus == '1' || item.miningStatus == '3'">
														<span class="icon-32 icon-32-btc"
															style="width: 24px;height: 24px;">
															<img class="theme-night-display-hide pc-header-logo icon-322"
																src="@/starchainminer/static/image/kaishi.png">
							...
							<div style="height: 200px;"></div>
							<el-pagination @current-change="getMiningOrder" :currentPage="miningOrderPage.current"
								:page-size="miningOrderPage.size" layout="prev, pager, next"
								:prev-text="$t('bico.W115')" :next-text="$t('bico.W116')"
						...
						<el-tab-pane :label="$t('bico.W234')" name="third">
							<div class="contentmep5" v-show="miningOrderDetailPage.records.length > 0"
								v-for="(item, index) in miningOrderDetailPage.records" :key="item.id">
										...
										<div class="flex-1">
											<div class="card-btccc">
												...
												<div style="margin: 0px 20px 0 0;">
													<p class="overview-btn" @click="pick(item, 'chan')">
														{{ $t('bico.W235') }}</p>
												</div>
												<div>
													<p class="go-buy-btn mt16" @click="pick(item, 'jing')">
														{{ $t('bico.W236') }}</p>
												</div>
											</div>
											...
											<div class="flex-start">
												...
												<div class="flex-col" style="width: 170px;">
													...
													<div class="fs20 mt8">{{ item.cycleNumber }}
														{{ cycleTypeMap[item.cycleType] }}</div>
												</div>
												<div class="flex-col" style="width: 170px;">
													<span class="fs12 gray-text"
														style="border-bottom: 1px dashed; cursor: help;">{{ $t('bico.W237') }}</span>
													<div class="fs20 mt8 color-E64E62">{{ item.estimatedCapacity }} %
													</div>
												</div>
												<div class="flex-col" style="width: 170px;">
													<span class="fs12 gray-text"
														style="border-bottom: 1px dashed; cursor: help;">{{ $t('bico.W238') }}</span>
													<div class="fs20 mt8 color-E64E62">{{ item.estimatedStaticYield }} %
													</div>
												</div>
												<div class="flex-col" style="width: 170px;">
													<div class="fs12 gray-text">{{ $t('bico.W239') }}：</div>
													<div class="fs20 mt8">{{ item.miningedDays }} {{ $t('bico.W214') }}
													</div>
												</div>
												<div class="flex-col" style="width: 170px;">
													<div class="fs12 gray-text">{{ $t('bico.W240') }}</div>
													<div class="fs20 mt8">{{ item.chanShouyi }} {{ item.chanShouyiCoin }}
													</div>
												</div>
											...
											<div class="flex-start mt16">
												...
												<div class="flex-col" style="width: 170px;margin-top: -54px;">
													...
													<div class="fs20 mt8">{{ item.fuelEnergyNumber }}
														{{ item.fuelEnergyCoin }}</div>
												</div>
												<div class="flex-col" style="width: 170px;margin-top: -54px;">
													<div class="fs12 gray-text">{{ $t('bico.W242') }}</div>
													<div class="fs20 mt8">{{ item.energyLossNumber * item.miningedDays }}
														{{ item.energyLossCoin }}</div>
												</div>
												<div class="flex-col" style="width: 170px;margin-top: -54px;">
													<div class="fs12 gray-text">{{ $t('bico.W243') }}</div>
													<div class="fs20 mt8">{{ item.calculatingPowerNumber }}
														{{ item.calculatingPowerCoin }}</div>
												</div>
												<div class="flex-col" style="width: 170px;margin-top: -54px;">
                      ...
                      aaa.chanShouyiCoin = aaa.miningCoin
                        ...
                        -->
												...
												<div class="flex-col" style="width: 170px;margin-top: -54px;">
													...
													<div class="fs20 mt8">{{ item.kejingtaishouyi }}
														{{ item.kejingtaishouyiCoin }}</div>
												</div>
							...
							<div style="height: 200px;"></div>
							<el-pagination @current-change="getMiningDetailOrder"
								:currentPage="miningOrderDetailPage.current" :page-size="miningOrderDetailPage.size"
								layout="prev, pager, next" :prev-text="$t('bico.W115')" :next-text="$t('bico.W116')"
									...
									<el-col :span="6">
										<div class="grid-content anniujuzhong">
											<el-button class="transBtn"
												@click="closeDialog">{{ $t('bico.W253') }}</el-button>
										</div>
									</el-col>
									<el-col :span="6">
										<div class="grid-content anniujuzhong"
											style="padding-left: 40px;padding-right: 10px;">
											<el-button class="themeBtn" v-show="token"
												@click="submit">{{ $t('bico.W219') }}</el-button>
											<el-button class="themeBtn" v-show="!token"
												@click="$router.push('/login');">{{ $t('bico.W254')}}</el-button>
										</div>
									</el-col>
									<el-col :span="6">
										<div class="grid-content">
