            ...
            {{sysTime |  dateFormat(that)}}
				...
				<div class="option-tab">
					<div :class="{ 'option-tab-item': true, 'option-tab-current': item.symbol == currentCoinSymbol }"
						class="option-tab-item" v-for="item in coinList" :key="item.symbol" @click="changeCoin(item)">
						{{ item.name }}</div>
				</div>
								...
								<div class="panel panel_buy">
									<div class="bd bd_limited">
										<div class="result-panel">
											...
											<div>
												<TradeView :nowPrice="newPirce" :symbolValue="symbolsStr"
													:defaultfloatPrecision="interception" :type="0"
													@getValueDise="getValueDise" />
											</div>

											<div v-if="historyTab == 1">
												<div class="result-item" v-for="item in historyList" :key="item.id">
													...
													<div class="item-body">
														<el-tooltip effect="dark" placement="bottom">
															<img v-if="item.result == 1" style="width:25px;height:25px"
																src="../assets/zhang1.png" />
															<img v-if="item.result == 2" style="width:25px;height:25px"
																src="../assets/die1.png" />
															<img v-if="item.result == 3" style="width:25px;height:25px"
																src="../assets/ping1.png" />
															<div slot="content">
																<div style="line-height: 2.2;">
																	{{ $t('option.opentime') }}：{{ item.openTime |
																		dateFormat(that) }}</div>
																<div style="line-height: 2.2;">
																	{{ $t('option.closetime') }}：{{ item.closeTime |
																		dateFormat(that) }}</div>
																<div style="line-height: 2.2;">
																	{{ $t('option.openprice1') }}：{{ item.openPrice }} USDT
																</div>
																<div style="line-height: 2.2;">
																	{{ $t('option.closeprice') }}：{{ item.closePrice }} USDT
																</div>
																<div style="line-height: 2.2;">
																	{{ $t('option.buyupreward') }}：{{ item.totalBuy }} USDT
																</div>
																<div style="line-height: 2.2;">
																	{{ $t('option.buydownreward') }}：{{ item.totalSell }} USDT
																</div>
															</div>
														</el-tooltip>
													</div>
										...
										<div class="last-period" v-if="openingOption.id != 0">
											...
											<div class="period-content">
												...
												<div class="content-item color_green">
													<span>{{$t('option.buyupreward')}}: <b>{{openingOption.totalBuy|fixedScale(that)}}</b> USDT</span>
												</div>
												<div class="content-item color_red">
													<span>{{$t('option.buydownreward')}}: <b>{{openingOption.totalSell|fixedScale(that)}}</b> USDT</span>
												</div>
											...
											<div style="width:200px;float: right;height:40px;line-height: 36px;text-align: right;padding-right: 10px;margin-top: 3px;">
												<div style="margin-top:8px;">
													<el-progress :text-inside="true" :stroke-width="18" :percentage="d_percentage" status="warning"></el-progress>
												</div>
											</div>
												...
												<div style="width: 100%;margin-top: 20px;">
													<div style="width: 50%;float:left;text-align: center;">
														<Button v-if="isLogin" class="bg-green"
															...
															size="large" @click="addClick(0)">{{ syTime
																<= 0 ? $t('option.fp') : $t('option.buyup') + ' ' + syTime + ' S' }}</Button>
													...
													<div style="width: 50%;float:right;text-align: center;">
														<Button v-if="isLogin" class="bg-red"
															style="width: 80%;height: 40px;line-height: 3;border-radius: 4px;"
															size="large" @click="addClick(1)">{{ syTime
																<= 0 ? $t('option.fp') : $t('option.buydown') + ' ' + syTime + ' S' }}</Button>
																<Button v-if="!isLogin" class="bg-gray"
																	style="width: 80%;height: 40px;line-height: 3;border-radius: 4px;"
																	size="large">{{ $t('option.login') }}</Button>
													</div>
...
import {
	realListApi, ticketApi, walletApi, coinOptionListApi, coinInfoApi, klineApi, sysTimeApi,
	historyListApi, openingApi, startingApi, orderCurrentApi, optionOrderAdd, orderHistoryApi, timeCountApi
} from '@/api/getData'
...
export default {
	components: { TradeView, expandRow, Foot, Datafeeds, $ },
	data() {
		return {
			sysTime: '',
			symbolsStr: 'BTC/USDT',
	...
	methods: {
		async getRealList() {
			...
			if (res.success) {
				...
				this.newPirce = dataList[0].price.toFixed(this.interception)
		...
		dateFormatHM2(tick) {
			var time = new Date(tick.createTime + " GMT+8"); //创建一个Date对象
			...
			var offset = (new Date().getTimezoneOffset() / (-60)); //以夏威夷时间为例，东10区

			var hawaii = utc + (3600000 * offset);

			var time = new Date(hawaii);
		...
		async ticketFun() {//获取币种
			...
			if (res.success) {
				...
				obj.forEach((element, index) => {
					if (element.symbol != '') {
						if (this.currentCoinSymbol == element.symbol) {
							this.currentPrice = (element.close).toFixed(this.currentCoin.coinScale);
						}
					}
				});
			} else {
		...
		async getCoinList() {
			...
			if (res.data.success) {
				...
				this.changeCoin(a ? this.coinList[0] : having)
			}
		},
		selectCount(count) {
