  ...
  export default {
    ...
    methods: {
      ...
      subscribeBars(symbolInfo, resolution, onRealtimeCallback, subscriberUID, onResetCacheNeededCallback) {
        ...
        if(that.timerY != null){
          that.timerY = clearInterval(that.timerY);
        }
        that.webSocket();
      },
      ...
      getSymbol(symbol) {
        ...
        return {
          ...
          'description': symbol,
          'pricescale':  10**this.defaultfloatPrecision,//刻度值,保留小数，如：2位小数传100，
          'ticker': symbol,
        }
      },
      ...
      loadChart() {
        ...
        this.chart.onChartReady(function() {//创建tradingview图
          try {
            ...
            //self.chart.chart().createStudy('指数平滑异同移动平均线(MACD_Custom)', false, false, [20], null, {});//自定义MACD,底部涨幅条
            //self.chart.chart().createStudy('MACD', false, false, [12], null, {});//自定义MACD,底部涨幅条
          ...
          self.$nextTick(async () => {
            ...
            if(result.success){
              ...
              if(arrLength>0){
                ...
                //obj[arrLength-1].close=self.nowPrice
                obj.forEach((item,index)=>{
                  if(index == (arrLength-1)){
                    if(self.newId !=item.id){
                      var item222 = obj[arrLength-2]
                      let dise = {
                        time: (item222.id) * 1000,
                        open: item222.open,
                        close: item222.close,
                        high: item222.high,
                        low: item222.low,
                        volume: item222.amount
                      };
                      //item.close=self.nowPrice
                      // console.log(dise);
                  ...
                  //if (obj.length>0) {
                  //obj[obj.length-1].close=self.nowPrice
                  // console.log(self.symbol+'最新价格'+self.nowPrice)
                  //}
    ...
    beforeDestroy(){
      ...
      self.timerY=clearInterval(self.timerY);
    }
  }
</script>
<style>
