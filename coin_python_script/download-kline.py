...
def download_daily_klines(trading_type, symbols, num_symbols, intervals, dates, start_date, end_date, folder, checksum):
  ...
  for symbol in symbols:
    print("[{}/{}] - 开始每日下载{}klines".format(current+1, num_symbols, symbol))
    for interval in intervals:
      for date in dates:
    ...
    current += 1
#  -skip-monthly -t spot  -s BTCUSDT ETHUSDT TRXUSDT LTCUSDT BCHUSDT EOSUSDT ETCUSDT XRPUSDT VETUSDT ADAUSDT UNIUSDT SOLUSDT ICPUSDT XLMUSDT XMRUSDT FILUSDT HOTUSDT -i 1m 5m 15m 30m 1h 1d 1w 1mo
if __name__ == "__main__":
    parser = get_parser('klines')
    args = parser.parse_args(sys.argv[1:])

    if not args.symbols:
      print("从exchange获取所有符号")
