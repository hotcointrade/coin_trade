...
Compact.initColumn = function () {
    return [[
        // {type: 'checkbox'},
         {field: 'compactId', hide: true, sort: true, title: 'id'},
    ...
    Compact.search = function () {
        ...
        queryData['compactId'] = Compact.condition.compactId;
        table.reload(Compact.tableId, {where: queryData});
        $.ajax({
            type:"post",
            url:Feng.ctxPath +"/compact/searchStatistics",
            data:queryData,
            dataType:'json',
...
Compact.openCompactDetail = function () {
    if (this.check()) {
        var index = layer.open({
            ...
            fix: false, //不固定
            maxmin: true,
            content: Feng.ctxPath + '/compact/compact_edit/' + Compact.seItem.id
        });
    ...
    Compact.onDeleteCompact = function (data) {
        var operation = function () {
            var ajax = new $ax(Feng.ctxPath + "/compact/delete", function () {
                Feng.success("删除成功!");
                table.reload(Compact.tableId);
            }, function (data) {
                Feng.error("删除失败!" + data.responseJSON.message + "!");
            });
            ajax.set("compactId", data.compactId);
            ajax.start();
        };
    ...
    Compact.openAddCompact = function () {
        ...
        top.layui.admin.open({
            ...
            title: '添加合约订单',
            area:["800px","420px"],
            content: Feng.ctxPath + '/compact/compact_add',
            end: function () {
     ...
     */
     Compact.onEditCompact = function (data) {
        ...
        top.layui.admin.open({
            ...
            title: '修改合约订单',
            area:["800px","420px"],
            content: Feng.ctxPath + '/compact/compact_edit?compactId=' + data.compactId,
            end: function () {
                admin.getTempData('formOk') && table.reload(Compact.tableId);
            }
        });
