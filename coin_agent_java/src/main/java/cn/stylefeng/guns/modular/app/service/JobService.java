    ...
    public void pl()
    {
        synchronized (this)
        {
            ...
            for (Compact entity : compactList)
            {
                ...
                //平仓价/开仓价
//                BigDecimal outDivTrade = close.subtract(entity.getTradePrice());
                BigDecimal outDivTrade = close;
                //杠杆*本金
//                BigDecimal levMulNum = entity.getLeverRate().multiply(entity.getNumbers());
    ...
    private BigDecimal getClosePrice(String symbols, String type) {
        ...
        return map == null ? BigDecimal.ZERO : new BigDecimal(map.getClose());
    }
        ...
        synchronized (this) {
            ...
            for (Compact dto : list) {
                ...
                if (StrUtil.equals(dto.getDealWay(), DealWay.LIMIT.code)
                        ...
                        && dto.getUnit().compareTo(getClosePrice(dto.getSymbols(), KINE_PERPETUAL)) <= 0) {
...
private AtomicReference<Thread> sign =new AtomicReference<>();
    ...
    public void contractUpdateCoin() {
        List<Swap> swapList = F.me().getSwaps("N");
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public void stopFuturesPl(String contractType) throws Exception {
        synchronized (this) {
            FuturesCompact compactQ = new FuturesCompact();
            compactQ.setStatus(CompactStatus.N.code).setCoin(contractType);
            List<FuturesCompact> compactList = futuresCompactService.list(new QueryWrapper<>(compactQ));
            for (FuturesCompact entity : compactList) {
                //提早出局未设置止盈止损，提高运行效率
                if (entity.getStopLoss() == null && entity.getStopProfit() == null)
                    continue;
                //获取最新行情价
                BigDecimal close = getClosePrice(entity.getSymbols(), KINE_FUTURES);
                //平仓价/开仓价
                BigDecimal outDivTrade = BigDecimal.ZERO;
                //杠杆*本金
                boolean flag = false;
                ...
                String coin = contractType;
                //配资账户
                FinFutures contract = F.me().getFinFutures(entity.getMemberId(), contractType);
                BigDecimal usedPrice = contract.getUsedPrice().add(totalReturnPrice).compareTo(BigDecimal.ZERO) <0
                        ? BigDecimal.ZERO :contract.getUsedPrice().add(totalReturnPrice);
                if (totalPlPrice.compareTo(BigDecimal.ZERO) > 0) {
                    F.me().saveCashflow(entity.getMemberId(), WalletBigType.FUTURES, CashFlowOpEnum.FLOW_IN, cashFlowTypeEnum,
                            totalPlPrice, coin, totalPlPrice, coin, outFee, coin,
                            ItemCode.USED, coin, null, "盈利",
                            contract.getUsedPrice(), usedPrice,
                            entity.getMemberId(), entity.getMemberId());
                } else if (totalPlPrice.compareTo(BigDecimal.ZERO) <0){
                    F.me().saveCashflow(entity.getMemberId(), WalletBigType.FUTURES, CashFlowOpEnum.FLOW_OUT, cashFlowTypeEnum,
                            BigDecimal.ZERO.subtract(totalPlPrice), coin, totalPlPrice, coin, outFee, coin,
                            ItemCode.USED, coin, null, "亏损",
                            contract.getUsedPrice(), usedPrice,
                            entity.getMemberId(), entity.getMemberId());
                ...
                contract.setUpdateUser(-1L);
                contract.setUpdateTime(new Date());
                int rows = finFuturesService.updateWallet(contract);

                //log.info("止盈止损平仓:id:{} 单号：{},类型：{}", entity.getCompactId(), entity.getOrderNo(), entity.getExitType());

                refreshFinFuturesInfo(entity.getMemberId(), coin);
                //添加一个处理单位
                redisUtil.lSet(OP_FINFUTURES_DATA, entity.getMemberId());
...
//                }
    ...
    @Transactional(rollbackFor = Exception.class)
    @Async("boomTask")
    public void boomFutures(String contractType) throws Exception {
        long start = System.currentTimeMillis();
        synchronized (this) {
            //保证金率
            BigDecimal contractMargin = new BigDecimal(F.me().getSysConfigValueByCode(CONTRACT_MARGIN)).divide(new BigDecimal(100));
            //获取当前合约订单用户ID列表
            List<Long> memberIdList = futuresCompactService.getBaseMapper().getMemberIdList();
            for (Long memberId : memberIdList) {
                //获取用户所有合约记录
                FuturesCompact compactQ = new FuturesCompact();
                compactQ.setDel("N")
                ...
                String coin = contractType;
                //获取当前净值
                FinFutures currentContract = (FinFutures) redisUtil.get(String.format(FINFUTURES_CODE, contractType) + memberId);
                for (FuturesCompact entity : compactList) {
                    BigDecimal lossPL = BigDecimal.ZERO;
                    Object o = redisUtil.get(PLF + entity.getOrderNo());
                    if (o != null){
                        lossPL = (BigDecimal) o;
                    }
                    log.info("黄金监听该订单号【"+entity.getOrderNo()+"】的可用余额与持仓保证金总和跟亏损比较"+
                            currentContract.getUsedPrice().add(entity.getPositionPrice()).add(
                                    lossPL.compareTo(BigDecimal.ZERO) <0
                                            ? BigDecimal.ZERO  : lossPL)+
                            ",可用余额："+currentContract.getUsedPrice()+",持仓保证金："+entity.getPositionPrice()+
                    ...
                    BigDecimal marginPrice = entity.getNumbers().multiply(
                            ...
                            .multiply(contractMargin);
                    if (entity.getPositionPrice().compareTo(marginPrice) <= 0) {
                        log.info("黄金该订单号【"+entity.getOrderNo()+"】超过了维持保证金======此时的计算后的维持保证金为"+marginPrice+
                                ",可用余额："+currentContract.getUsedPrice()+",持仓保证金："+entity.getPositionPrice());
                        isBoom = true;
                    }
                    ...
                    if (!isBoom && currentContract.getUsedPrice().compareTo(BigDecimal.ZERO) <=0){
                        BigDecimal oldLossPrice = BigDecimal.ZERO;
                        BigDecimal subPrice = totalPlPrice;
                        if(redisUtil.get("OLD_LOSS_PLF"+entity.getOrderNo()) != null){
                            oldLossPrice = (BigDecimal) redisUtil.get("OLD_LOSS_PLF"+entity.getOrderNo());
                        }
                        if (oldLossPrice.compareTo(BigDecimal.ZERO) ==0 ){
                            if (subPrice.compareTo(BigDecimal.ZERO) >=0){
                                subPrice = BigDecimal.ZERO;
                        ...
                        log.info("黄金该订单号【"+entity.getOrderNo()+"】======补差值："+subPrice+",老的盈亏额："+oldLossPrice+
                                ",新盈亏额："+totalPlPrice+
                                ",可用余额："+currentContract.getUsedPrice()+",持仓保证金："+entity.getPositionPrice());
                        if (subPrice.compareTo(BigDecimal.ZERO) <0 && entity.getPositionPrice().add(subPrice).compareTo(BigDecimal.ZERO) <=0){
                            log.info("黄金该订单号【"+entity.getOrderNo()+"】持仓保证金不够扣除，进行爆仓======补差值："+subPrice+",老的盈亏额："+oldLossPrice+
                                    ",新盈亏额："+totalPlPrice+
                                    ",可用余额："+currentContract.getUsedPrice()+",持仓保证金："+entity.getPositionPrice());
                            isBoom = true;
                        }
                    ...
                    if (isBoom){
                        ...
                        futuresCompactService.updateById(entity);
                        //配资账户
                        FinFutures entrust = F.me().getFinFutures(memberId, contractType);
                        //更新余额
                        BigDecimal usedPrice = entrust.getUsedPrice().add(totalReturnPrice).compareTo(BigDecimal.ZERO) <0
                                ? BigDecimal.ZERO :entrust.getUsedPrice().add(totalReturnPrice);
                        if (totalPlPrice.compareTo(BigDecimal.ZERO) > 0) {
                            F.me().saveCashflow(entity.getMemberId(),
                                    WalletBigType.FUTURES,
                                    CashFlowOpEnum.FLOW_IN,  CashFlowTypeEnum.FIXED,
                                    totalPlPrice, coin, totalPlPrice, coin, outFee, coin,
                                    ItemCode.USED, coin, null, "盈利",
                                    entrust.getUsedPrice(), usedPrice,
                            ...
                            F.me().saveCashflow(entity.getMemberId(), WalletBigType.FUTURES, CashFlowOpEnum.FLOW_OUT, CashFlowTypeEnum.FIXED,
                                    BigDecimal.ZERO.subtract(totalPlPrice), coin, totalPlPrice, coin, outFee, coin,
                                    ItemCode.USED, coin, null, "亏损",
                                    entrust.getUsedPrice(), usedPrice,
                                    entity.getMemberId(), entity.getMemberId());
                        ...
                        while (true){
                            if (finFuturesService.updateWallet(entrust) >0){
                                refreshFinFuturesInfo(entrust.getMemberId(), "USDT");
                                break;
                            }
                        }
...
//        }
    ...
    @Transactional(rollbackFor = Exception.class)
    @Async("asyncTaskExecutor")
    public void futuresUpdateCoin(String contractType) {

        String code = contractType;
        //获取当前黄金订单用户ID列表
        boolean lock = redisUtil.lock(FUTURES_UPDATE_LOCK, 1, 60);
        if (!lock) {
            //log.info("黄金刷新-未获取锁");
            return;
        }
        ...
        //止盈止损及爆仓处理用户队列
        List many = redisUtil.lGet(OP_FINFUTURES_DATA, 0, -1);

        for (Object o : many) {
            Long id = (Long) redisUtil.lPop(OP_FINFUTURES_DATA);
            refreshFinFuturesInfo(id, code);
        }

        List<Long> coMembers = futuresCompactService.getBaseMapper().getMidsByInAndN();
        for (Long memberId : coMembers) {
            refreshFinFuturesInfo(memberId, code);
        }

        redisUtil.unLock(FUTURES_UPDATE_LOCK);

    }
    public void plFuturesTotal(String coin) {
        // log.info("{}盈亏监听", coin);
        synchronized (this) {
            //获取当前合约订单用户ID列表
            List<Long> memberIdList = futuresCompactService.getBaseMapper().getMemberIdList();
            for (Long memberId : memberIdList) {
                ...
                for (FuturesCompact entity : compactList) {
                    ...
                    redisUtil.set(PLF + entity.getOrderNo(), pl.setScale(4, RoundingMode.DOWN), EXPIRE_NORMAL);
                    totalPlPrice = totalPlPrice.add(pl);
                }
                log.info("用户【"+memberId+"】总盈亏额："+totalPlPrice.toPlainString());
                redisUtil.set(String.format(PLF_TOTAL_, coin) + memberId, totalPlPrice.toPlainString());
                this.redisUtil.set(String.format(LOSS_PLF_TOTAL_, coin) + memberId, lossTotalPlPrice.toPlainString());
            }
        }
    ...
    @Transactional(rollbackFor = Exception.class)
    public void futuresEntrust() throws Exception {
        synchronized (this) {
            //当前委托订单
            List<FuturesCompact> list = futuresCompactService.getBaseMapper().getEntrust();
            for (FuturesCompact dto : list) {
                //当前行情价
                //订单状态处理
                //限价卖出： 单价<=当前行情价 ，订单成交进入持仓
                if (StrUtil.equals(dto.getDealWay(), DealWay.LIMIT.code)
                        && StrUtil.equals(dto.getCompactType(), CompactType.SELL.code)
                        && dto.getUnit().compareTo(getClosePrice(dto.getSymbols(), KINE_FUTURES)) <= 0) {
                    dto.setStatus(CompactStatus.N.code);
                }
                //限价买入：单价>=行情价，订单成交
                if (StrUtil.equals(dto.getDealWay(), DealWay.LIMIT.code)
                ...
                dto.setUpdateUser(SYS_PLATFORM);
                int rows = futuresCompactService.getBaseMapper().updateEntrust(dto);
                if (rows == 0) {
//                    JSONObject jsonObject = new JSONObject();
//                    jsonObject.put("request", dto.toString());
//                    jsonObject.put("response", rows);
//                    PromotionFactory.me().saveWalletException("entrust", "合约 委托转持仓 更新异常", jsonObject);
                    throw new Exception("黄金更新异常");
                }
