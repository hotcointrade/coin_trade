...
//            }
    ...
    //计算推荐佣金
    private void calculateReferralCommission(Member member){
        ...
        if(parentRefereeId.length()>1){ //有上级
            ...
            //十级内计算一级佣金
            if(split.length<=10 && split.length!=0){

                Config config = conMap.get(split.length-1);
                Member m = memberService.getById(member.getRefereeId());
...
//                ()->{
//                    generateRechagerAddress(resultMember);
//                }
//        );
        //单账号登录
        ...
        for (Swap swap : swapList){
            //刷新合约用户账户
            jobService.refreshContractInfo(resultMember.getMemberId(), swap.getSymbol().split("/")[0]);
        }
            ...
            for (Spot spot1:spotList){
                if (StringUtils.isNotBlank(spot1.getCode())){
                    ...
                    if (memberRechargeAddress == null){
                        ...
                        if ("EOS".equals(spot1.getSymbol()) || "XRP".equals(spot1.getSymbol())){
                            String eosXrp = F.me().getSysConfigValueByCode(spot1.getSymbol()+"_ADDRESS");
                            if (StringUtils.isNotBlank(eosXrp)){
                                ...
                                memberRechargeAddress1.setCoin(spot1.getSymbol());
    ...
    private void generateRechagerAddress(Member member){
        boolean open = false;
        if (StrUtil.equals(F.me().cfg(CHAIN_OPEN), "Y")){
            open = true;
        }
    ...
    private void genOption(Member m) {
    ...
    public Result getMsg2(String token, String code, String phone, Long type, String email) {
        Result result;
        String account;
        int number = (int) ((Math.random() * 9 + 1) * 100000);
        String content = F.me().cfg(SMS_CONTENT);
        ...
        if ((!phone.contains("@")) && (!phone.equals("1"))){
            account = phone;
            result = sendSms(account, code, type, content, number);
        }else{
            account = email;
            content =  F.me().cfg("SMS_EMAIL_CONTENT");
            content = String.format(content, number);
    ...
    private IPage geCashList(Cashflow cashflow, Page page, String type, String msg,String languageType){
        ...
        tmpList.stream().forEach(entity ->
        {
            ...
            for (CashFlowTypeEnum e : CashFlowTypeEnum.values())
            {
                ...
                {
                    switch (languageType){
                        case "1" : dto.setFlowType(e.getValue());break;
                        case "2" : dto.setFlowType(e.getHkValue());break;
                        case "3" : dto.setFlowType(e.getHgValue());break;
                        case "4" : dto.setFlowType(e.getJpValue());break;
                        default :  dto.setFlowType(e.getUsValue());break;
                    }
...
//                .setType("USDT")
        ...
        if (vo.getValuation().equals("BTC")){
            Kline map = (Kline) redisUtil.get(KINE + BTC_USDT + _NEW);
            BigDecimal close = new BigDecimal(map.getClose());
...
//            dto.setP7("--");
    ...
    public Result platformAddress(String token,String symbol){
        ...
        platformAddress.setType(symbol);
        platformAddress.setDel("N");
        platformAddress.setStatus("Y");
        platformAddress = this.platformAddressService.getOne(new QueryWrapper<>(platformAddress));
        Map<String, Object> map = new HashedMap();
        map.put("address",null);
        map.put("memoTagValue",null);
...
//            returnNumber=matchR.getWill().subtract(matchR.getOk()).subtract(feePrice);
     ...
     * @return Result
     */
    public Result contractPage(String token, String symbols, String coin){
        ...
        leverage.setDel("N");
        List<Leverage>  list =leverageService.list(new QueryWrapper<>(leverage));
    ...
    public Result contract(String token, ApiContractDto dto) throws Exception{
        synchronized (this){
            ...
            //获取火币的最新行情价
            BigDecimal close = getClosePrice(dto.getSymbols(), KINE_PERPETUAL);
            ...
            BigDecimal fee = dto.getNumbers().multiply(new BigDecimal(swap.getHandNumber())).multiply(unit).multiply(contractFee);
            //开仓保证金=委托手数*每手价值数量*委托价格/杠杆倍数
            BigDecimal openPrice =  dto.getNumbers().multiply(new BigDecimal(swap.getHandNumber()))
    ...
    @Transactional(rollbackFor = Exception.class)
    public Result cancelContract(String token, Long compactId,Integer type) {
        Member member = (Member) redisUtil.get(token);
        if (type<=1){
    ...
    public Result contractPl(String token, ApiPlDto dto){
        ...
        //当前行情
        BigDecimal close = getClosePrice(compactR.getSymbols(), KINE_PERPETUAL);
     ...
     */
    @Transactional(rollbackFor = Exception.class)
    public Result outContract(String token, Long compactId, Integer type, BigDecimal number){
        synchronized (this) {
            Member member = (Member) redisUtil.get(token);
            if (type<=1){
                Compact compactQ = new Compact();
    ...
    public void dealOutContract(Compact compactR, BigDecimal number,Member member){
        ...
        //当前行情价
        BigDecimal currentUnit = getClosePrice(compactR.getSymbols(), KINE_PERPETUAL);
        //总盈亏
...
//        updateMemberChainAddress(member);
    ...
    public Result orderhistory(String token,
        ...
        contractOptionOrder.setMemberId(member.getMemberId());
        contractOptionOrder.setSymbol(symbol);
        return Result.success(optionOrderService.page(page,new QueryWrapper<>(contractOptionOrder).orderByDesc(CREATE_TIME)));
    }
...
////                swap.setDel("N");
    ...
    public Result teamDetail(String token,Page page) {
        ...
        Page<Map<String, Object>> mapPage = memberService.selectAllForTeam(null, member.getRefereeId(), u.getUserId(),page);
