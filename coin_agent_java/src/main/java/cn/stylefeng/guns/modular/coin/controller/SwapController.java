...
public class SwapController extends BaseController {
    ...
    private String PREFIX = "/modular/coin/swap/";
    ...
    @ResponseBody
    public ResponseData edit(Swap swap) {
  ...
  //  @BussinessLog(value = "添加合约交易对", key = "name", dict = SwapMap.class)
    @ResponseBody
    public ResponseData add(Swap swap, BindingResult result) {
        if (result.hasErrors()) {
   ...
   // @BussinessLog(value = "删除合约交易对", key = "swapId", dict = SwapMap.class)
    ...
    public ResponseData delete(@RequestParam Long swapId) {
        ...
        Swap swap = this.swapService.getById(swapId);
        this.swapService.deleteSwap(swapId);
        redisUtil.set("UPDATE_SWAP","Y");
        ThreadPoolUtil.execute(
                ()->{
                    updateContract("delete",swap.getSymbol());
                }
        );
        return SUCCESS_TIP;
    }
    ...
    public ResponseData status(@PathVariable("status") String status,@RequestParam Long id) {
        Swap entity = swapService.getById(id);
    ...
    public ResponseData getList() {
        ...
        swap.setDel("N");
        swap.setStatus("Y");
        List<Swap> list = 	swapService.list(new QueryWrapper<>(swap));
