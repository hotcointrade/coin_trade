...
import cn.stylefeng.guns.modular.coin.wrapper.ContractOptionOrderWrapper;
...
public class ContractOptionOrderController extends BaseController {
    ...
    public Object list(@RequestParam(required = false)String symbol,
                       @RequestParam(required = false)BigDecimal betAmount, @RequestParam(required = false)BigDecimal rewardAmount) {
        ...
        Page<Map<String, Object>> result = optionService.selectByCondition2( symbol, memberId,recommendIds, betAmount, rewardAmount);
        Page wrapped = new ContractOptionOrderWrapper(result).wrap();
